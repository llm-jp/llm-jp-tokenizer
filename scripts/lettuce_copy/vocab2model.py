...
def setVocabToTokenizer(tknzr, vocab, args):
    for token in vocab:
        ...
        if token[0] in args.special_tokens:
            newToken.type = newToken.CONTROL
        #elif ' ' in token[0]:
        #    print('[%s] -> CONTROL'%token[0])
            newToken.type = newToken.CONTROL
        elif token[0] == args.unk_token:
            newToken.type = newToken.UNKNOWN
        elif token[0].startswith('<0x') and token[0].endswith('>'):
...
def main():
    ...
    parser.add_argument('-st', '--special-tokens', nargs='+',
                        default=['<s>', '</s>','<mask>','<pad>','<CLS>','<EOD>','<SEP>'])
